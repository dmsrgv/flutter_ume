...
void main() {
  group('ColorSucker', () {
    ...
    testWidgets('ColorSucker Pan the toolbar', (tester) async {
      ...
      final Offset toolbarLocation = tester.getCenter(find.byWidgetPredicate((widget) {
        return widget is GestureDetector && widget.onVerticalDragUpdate != null;
      }));

      final TestGesture dragGesture = await tester.startGesture(toolbarLocation, pointer: 0);
      await tester.pump(const Duration(seconds: 3));
